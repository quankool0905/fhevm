...
async fn swap_request_whitepaper(
    ...
    for _ in 0..=(num_samples - 1) as u32 {
        ...
        allow_handle(pending_0_in_handle.clone(), &pool).await?;
        allow_handle(pending_1_in_handle.clone(), &pool).await?;
        allow_handle(pending_total_token_0_in.clone(), &pool).await?;
        allow_handle(pending_total_token_1_in.clone(), &pool).await?;
    }
...
async fn swap_claim_whitepaper(
    ...
    for _ in 0..=(num_samples - 1) as u32 {
        ...
        if total_dex_token_1_in != 0 {
            ...
            allow_handle(new_from_amount_handle_0.clone(), &pool).await?;
            allow_handle(new_to_amount_handle_0.clone(), &pool).await?;
        }
        if total_dex_token_0_in != 0 {
            ...
            allow_handle(new_from_amount_handle_1.clone(), &pool).await?;
            allow_handle(new_to_amount_handle_1.clone(), &pool).await?;
        }
    }
